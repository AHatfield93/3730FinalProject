...
def options_window(master):
    ...
    Label(top, text="Your IP:").grid(row=0)
    ipaddress = Entry(top)
    ipaddress.grid(row=0, column=1)
    ipaddress.focus_set()
    Label(top, text="Your MAC:").grid(row=1)
    mac = Entry(top)
    mac.grid(row=1, column=1)
    mac.focus_set()
    Label(top, text="Reciever IP:").grid(row=2)
    recieverIP = Entry(top)
    recieverIP.grid(row=2, column=1)
    recieverIP.focus_set()
    Label(top, text="Reciever MAC:").grid(row=3)
    recieverMac = Entry(top)
    recieverMac.grid(row=3, column=1)
    recieverMac.focus_set()
    Label(top, text="Port:").grid(row=4)
    port = Entry(top)
